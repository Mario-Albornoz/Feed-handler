...
func TestFallbackSelection(t *testing.T) {
	...
	for i := 0; i < 49; i++ {
		openStats.UpdateIntertick(10.0)
	}
...
func TestIlliquidInstrumentFallback(t *testing.T) {
	...
	for i, bucket := range sessionSequence {
		...
		state.StatsBySession[bucket].UpdateIntertick(28800000.0) // 8 hours in ms
		// Update fallback (this would happen in real ProcessTick)
		state.AllSessionStats.UpdateIntertick(28800000.0)
...
func TestBimodalInstrument(t *testing.T) {
	...
	for i := 0; i < 500; i++ {
		...
		openStats.UpdateIntertick(50.0)
		// Also update fallback
		state.AllSessionStats.UpdateIntertick(50.0)
	}
...
func TestFallbackAlwaysUpdates(t *testing.T) {
	...
	state.StatsBySession[Overnight].UpdateIntertick(28800000.0)
	state.AllSessionStats.UpdateIntertick(28800000.0)
